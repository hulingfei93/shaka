# -*- coding: utf-8 -*-
"""
Created on Sat Nov 07 00:08:44 2015
        ...
        while i < word_list_len:
            ...
            if word in negation_dict:
                ...
                """
                    ...
                    if next_next_word in positive_dict:
                        word_list2.append('negative')
                        word_list2.append(next_word)
                        i = i + 3
                    elif next_next_word in negative_dict:
                        word_list2.append('positive')
                        word_list2.append(next_word)
                        i = i + 3
                    else:
...
# print preprocess_v2('no the waste not poise never')
...
def load_data(data_file_path):
    ...
    for line in data_file:
        ...
        reviews.append(preprocess_v2(review))
...
def train (name, pipeline, grid_parameters, train_data, train_targets):
    print "---------------------" + name + "---------------------"
    gs = GridSearchCV(cv = 5, estimator = pipeline, param_grid = grid_parameters, n_jobs = -1) 
    gs.fit(train_data, train_targets)
    
    best_parameters, best_score, _ = max(gs.grid_scores_, key=lambda x: x[1])
    print "best score: " + str(best_score)
    print "----best parameters----"
    for parameter_name in sorted(grid_parameters.keys()):
...
if __name__ == '__main__':
    # load training data
    train_reviews, train_targets = load_data('training_all.txt')
    
    """
    # Classifier implementing the k-nearest neighbors vote
    KNN_pipeline = Pipeline([('vect', CountVectorizer(stop_words = None)), 
                             ('tfidf', TfidfTransformer(norm = 'l2', use_idf = True, smooth_idf = True, sublinear_tf = True)), 
                             ('clf', KNeighborsClassifier()),
                            ])
    KNN_parameters = {'vect__ngram_range': [(1, 1), (1, 2)],           
                      'clf__n_neighbors': (20, ),
                     }
    
    train('KNeighborsClassifier', KNN_pipeline, KNN_parameters, train_reviews, train_targets)
    """
    
    
    # Naive Bayes classifier for multinomial models
    MNB_pipeline = Pipeline([('vect', CountVectorizer(stop_words = None)), 
                             ('clf', MultinomialNB()),
                            ])
    MNB_parameters = {'vect__ngram_range': [(1, 1)],
                      'clf__alpha': (1.0, ),
                      'clf__fit_prior': (True, ),
                     }
    
    train('MultinomialNB', MNB_pipeline, MNB_parameters, train_reviews, train_targets)
    
    
    
    # Linear classifiers (SVM, logistic regression, a.o.) with SGD training
    SGD_pipeline = Pipeline([('vect', CountVectorizer(stop_words = None)), 
                             ('tfidf', TfidfTransformer(norm = 'l2', use_idf = True, smooth_idf = True, sublinear_tf = True)), 
                             ('clf', SGDClassifier(loss = 'hinge', penalty = 'l2', l1_ratio = 0, n_iter = 5, warm_start = True)),
                            ])
    ...
    """
     # Perceptron
    Perceptron_pipeline = Pipeline([('vect', CountVectorizer(stop_words = None)), 
                                    ('tfidf', TfidfTransformer(norm = 'l2', use_idf = True, smooth_idf = True, sublinear_tf = True)), 
                                    ('clf', Perceptron()),
                                   ])
    
    Perceptron_parameters = {'vect__ngram_range': [(1, 2)],
                             'clf__penalty': (None, ),
                             'clf__warm_start': (True, ),
                            }
                     
    train('Perceptron', Perceptron_pipeline, Perceptron_parameters, train_reviews, train_targets)
    """
    ...
    KNN_pipeline = Pipeline([('vect', CountVectorizer(ngram_range = (1, 2))), 
                             ('tfidf', TfidfTransformer(norm = 'l2', use_idf = True, smooth_idf = True, sublinear_tf = True)), 
    ...
    DT_pipeline = Pipeline([('vect', CountVectorizer(ngram_range = (1, 1))), 
                            ('tfidf', TfidfTransformer(norm = 'l2', use_idf = True, smooth_idf = True, sublinear_tf = True)), 
                            ('clf', DecisionTreeClassifier(max_depth = None, max_features = None, class_weight = None)),
                           ])
                        
    VC = VotingClassifier(estimators=[('MNB', MNB_pipeline), ('SGD', SGD_pipeline), ('KNN', KNN_pipeline), ('LR', LR_pipeline), ('RFC', RFC_pipeline), ('DT', DT_pipeline)], voting = 'soft', weights = [2, 2, 1, 1, 1, 1])
    
    
    for clf, label in zip([MNB_pipeline, SGD_pipeline, KNN_pipeline, LR_pipeline, RFC_pipeline, DT_pipeline, VC], ['MNB', 'SGD', 'KNN', 'LR', 'RFC', 'DT', 'Ensemble']):
        scores = cross_validation.cross_val_score(clf, train_reviews, train_targets, cv = 5, scoring = 'accuracy')
        print("Accuracy: %0.2f (+/- %0.2f) [%s]" % (scores.mean(), scores.std(), label))
    """
    
    """
    VC_parameters = {'voting': ('hard', ),
                    }

    train('VotingClassifier', VC, VC_parameters, train_reviews, train_targets)
    """
